...
void error_sub_change_cmd_history_basic(i_s *info_shell, char *nbr)
{
    if (my_strlen(nbr) > 9)
...
char *sub_change_cmd_history_basic(i_s *info_shell, char *tmp, int n)
{
    ...
    if (my_strlen(nbr) >= 0) {
        ...
        error_sub_change_cmd_history_basic(info_shell, nbr);
    }
    free(nbr);
    return tmp;
}
...
char *change_cmd_history_basic(i_s *info_shell, char *tmp)
{
    ...
    for (int n = 0; my_strlen(tmp) > n; n++) {
        if (tmp[n] == '!' && (tmp[n + 1] > '0' && tmp[n + 1] < '9')) {
            tmp = sub_change_cmd_history_basic(info_shell, tmp, n);
        }
        if (n > my_strlen(tmp))
            return tmp;
    }
